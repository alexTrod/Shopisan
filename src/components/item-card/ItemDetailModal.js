...
const ItemDetailModal = ({ visible, onClose, item }) => {
  ...
  const isFavorite = useSelector((state) =>
    state.user.favoriteStores.includes(item.id),
  );
  ...
  const address = (itemAddress) => {
    const location =
      itemAddress.length > 0
        ? itemAddress[0].location
        : {
            address: { street: "" },
  ...
  const fetchPostMedia = async () => {
    try {
      const posts = await PostService.getPostsForDisplay(item.id);
      setPostMedia(posts);
    } catch (error) {
  ...
  useEffect(() => {
    if (!visible) {
      // Reset cache when modal closes - ensures fresh data on next open
    ...
    try {
      ...
      if (!geopoint) {
        Alert.alert(
          ...
          t("no_store_coordinates") ||
            "This store has no coordinates to show on the map.",
        );
        return;
      }
      ...
      if (isNaN(latitude) || isNaN(longitude)) {
        Alert.alert(
          ...
          t("invalid_store_coordinates") ||
            "This store has invalid coordinates.",
        );
        return;
      }
        ...
        <View style={styles.sheet}>
          ...
          >
            ...
            <View style={styles.compactRating}>
              ...
              <Text style={styles.yourRatingLabel}>
                {t("your_rating_short")}:
              </Text>
              <View style={styles.starsRow}>
                {[...Array(5)].map((_, i) => (
