...
export default function Map({ navigation, route }) {
  ...
  const fetchNearbyStores = useCallback(
    (location, useExpandingRadius = false) => {
  ...
  useEffect(() => {
    const initializeMap = async () => {
      ...
      if (
        hasInitializedWithLocationRef.current &&
      ...
      try {
        ...
        if (initialStore?.latitude && initialStore?.longitude) {
          console.log(
            "[Map] Using initial store location:",
        ...
        if (customLocation?.latitude && customLocation?.longitude) {
          console.log(
            "[Map] Using customLocation from Redux:",
        ...
        if (location) {
          console.log(
            "[Map] Using location from context:",
            location.latitude,
            location.longitude,
            location.source,
          );
          ...
          if (nearbyStores.length === 0) {
            Toast.show({
              ...
              text2:
                ...
                "Add your favorite shops or explore another city",
              position: "bottom",
              visibilityTime: 4000,
            });
          }
          ...
          console.log(
            "[Map] userLocation not ready, using default Brussels temporarily",
          );
          setCameraCoordinates({
   ...
   * Note: Callback must be sync for Hermes release builds - async logic wrapped inside
   */
  ...
  useEffect(() => {
    if (initialStore && stores.length > 0) {
      const matchingStore = stores.find(
        (store) => store.id === initialStore.id,
      );
      if (matchingStore) {
            ...
            <MapboxGL.Camera
              ...
              centerCoordinate={[
                cameraCoordinates?.longitude ||
                  LOCATION_CONFIG.DEFAULT_LOCATION.longitude,
                cameraCoordinates?.latitude ||
                  LOCATION_CONFIG.DEFAULT_LOCATION.latitude,
              ]}
            />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSubtitle}>
              {t("no_stores_modal_description") ||
                "There are no stores in your current location."}
            </Text>
