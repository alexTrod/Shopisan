...
export const signUp = (email, username, password, userType) => async (dispatch) => {
  try {
    ...
    return { ok: true, uid: new_id };
  } catch (error) {
    let message = 'Signup failed. Please try again.';
    if (error.code === 'auth/email-already-in-use') message = 'The email address is already in use by another account.';
    ...
    else if (error.code === 'auth/weak-password') message = 'The password is too weak.';
