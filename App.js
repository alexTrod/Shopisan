...
import ManagePosts from "./src/screens/app/manage_posts";
...
const SplashScreen = () => {
  ...
  const tagline =
    global.i18n?.t?.("splash_tagline") ||
    "Shop local, shop different, discover original";
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const checkI18n = () => {
      ...
      try {
        ...
        if (global.i18n) {
          console.log(
            "App.js: global.i18n properties:",
            Object.keys(global.i18n),
          );
          console.log(
            "App.js: global.i18n.isInitialized exists:",
            !!global.i18n.isInitialized,
          );
        }
    ...
    runCitiesMigration()
      .then((result) => {
        if (result.success) {
          ...
          console.log(
            "ℹ️ Cities migration not needed or failed:",
            result.message,
          );
        }
      })
  ...
  if (loading || !i18nReady) {
    console.log("🔴 Showing SPLASH because:", {
      loading,
      i18nReady: !i18nReady,
    });
  ...
  console.log(
    "🟢 Showing NAVIGATION - auth screens:",
      ...
      >
        {isAuthenticated || noAuthenticationWanted ? (
          <>
            ...
            />
            <Stack.Screen
              name="SuggestIdeaScreen"
              component={SuggestIdeaScreen}
            />
            <Stack.Screen
...
const WrappedApp = () => {
  ...
  return (
    <Provider store={store}>{showSplash ? <SplashScreen /> : <App />}</Provider>
  );
};
