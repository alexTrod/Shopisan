...
/**
 * Standalone Cities Migration Script
 * Run this script to migrate cities.json data to Firestore
 * 
 * Usage:
...
// Cities data from the JSON file
const cities = [
...
async function migrateCities() {
  try {
    ...
    const existingCities = await getDocs(citiesRef);
    
    if (existingCities.docs.length > 0) {
      console.log(`⚠️ Found ${existingCities.docs.length} existing cities. Skipping migration.`);
      return;
    }
    ...
    for (const cityName of cities) {
      try {
        const countryId = countryMapping[cityName] || 'FR';
